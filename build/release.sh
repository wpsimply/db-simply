#!/bin/sh
#
# Build the release archive: only what a server needs to run DB Simply.
#
# Usage: build/release.sh <version> [output-dir]
#
# Produces <output-dir>/db-simply-<version>.zip (and a .sha256 next to it)
# with a single top-level db-simply/ directory. Tests, examples, CI and
...
mkdir -p "$app/storage/sessions" "$app/storage/sso-tokens" "$app/storage/imports"
chmod 700 "$app/storage/sessions" "$app/storage/sso-tokens" "$app/storage/imports"

# Nothing from a developer's machine or git may ride along.
find "$app" \( -name '.git*' -o -name '.DS_Store' -o -name '*.swp' \) -exec rm -rf {} +

archive="$out_dir/db-simply-$version.zip"
rm -f "$archive"
(cd "$stage" && zip -qrX "$archive" db-simply)
