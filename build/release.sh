#!/bin/sh
#
# Build the release archive: only what a server needs to run DB Admin.
#
# Usage: build/release.sh <version> [output-dir]
#
# Produces <output-dir>/db-admin-<version>.zip (and a .sha256 next to it)
# with a single top-level db-admin/ directory. Tests, examples, CI and
...
archive="$out_dir/db-admin-$version.zip"
rm -f "$archive"
(cd "$stage" && zip -qrX "$archive" db-admin)
