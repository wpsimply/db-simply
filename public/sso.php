...
if (isset($_GET['start'])) {
    ...
    if (! is_string($issueUrl) || $issueUrl === '') {
        $fail('Sign-on from DB Simply is not set up. Open DB Simply from your control panel.', 404);
    }
...
header('Location: ./'.($grant['database'] !== null ? '?'.http_build_query(['db' => $grant['database']]) : ''));
